...
def main(input_file):
    ...
    result = []
    #visited = bfs(puzzle, ((0, 3), (1, 0)))
    #print(len(unique_visited(visited)))
    for i in range(0, puzzle.shape[0]):
        visited1 = bfs(puzzle, ((0, i), (1, 0)))
        visited2 = bfs(puzzle, ((puzzle.shape[1] - 1, i), (-1, 0)))
        result.append(len(unique_visited(visited1)))
        result.append(len(unique_visited(visited2)))
    for j in range(0, puzzle.shape[1]):
        visited1 = bfs(puzzle, ((j, 0), (0, 1)))
        visited2 = bfs(puzzle, ((j, puzzle.shape[0] - 1), (0, -1)))
        result.append(len(unique_visited(visited1)))
        result.append(len(unique_visited(visited2)))
    print(max(result))
